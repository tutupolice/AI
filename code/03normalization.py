...
def main():
    ...
    # 验证是否所有特征都在[0,1]区间
    all_in_range = ((min_values >= 0.0).all() and (max_values <= 1.0).all() and 
                    (min_values <= 0.1).all() and (max_values >= 0.9).all())
    print(f'所有特征正确归一化到[0,1]区间: {all_in_range}')
    print()
